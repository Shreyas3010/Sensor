...
a1=0
#A=[0.5,0.4,0.3,0.2,0.1,0.09,0.08,0.07,0.06,0.05,0.04,0.03,0.02,0.01]
    ...
    if rat1<=1:
        ...
        for i2 in B:
            ...
            if rat2 <=1:
                ...
                data_train3=data_train3.replace({'machine_status': mapping3})
                data_train3= data_train3.drop(['Class'],axis=1)
                data_train3.to_csv(r'sensor_SMOTE_ratio_broken_'+i1str+'_ratio_rec_'+i2str+'_RUS_'+i3str+'_.csv')
                mapping4 = {'NORMAL':0 ,'RECOVERING':1 ,'BROKEN':2}
                data_train3=data_train3.replace({'machine_status': mapping4})


                ratio1=str(i1)
                ratio2=str(i2)
                X = data_train3.loc[:, data_train3.columns != 'machine_status']
                y = data_train3.loc[:, data_train3.columns == 'machine_status']

                #split
                X_train, X_test, y_train, y_test = train_test_split(X,y,test_size = 0.3,train_size=0.7, random_state = 0)

                print("datasize",datasize)
                sampleddatasize=collections.Counter(data_train3['machine_status'])
